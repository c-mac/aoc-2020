...
const isValidPart1 = (min, max, char, password): boolean => {
  const charCount = password.split('').filter(letter => letter === char).length;
  return charCount >= min && charCount <= max;
}

const isValidPart2 = (pos1, pos2, char, password): boolean => {
  const passArray = password.split('');
  return (
    (passArray[pos1 - 1] === char && passArray[pos2 - 1] !== char) ||
    (passArray[pos1 - 1] !== char && passArray[pos2 - 1] === char)
  );
}

const val = data.reduce((accumulator, current) => {
  const [_, min, max, char, password] = current.toString().match(regex);
  // const value = isValidPart1(min, max, char, password) ? 1 : 0;
  const value = isValidPart2(min, max, char, password) ? 1 : 0;
  console.log(_, min, max, char, password, value);
